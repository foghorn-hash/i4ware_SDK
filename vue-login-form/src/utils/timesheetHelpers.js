...
export const makeRow = (id) => ({
  id,
  status: 'Luotu',
  project: '', pvm: '', klo_alku: '00:00', klo_loppu: '00:00',
  norm: 0, lisatLa: 0, lisatSu: 0, lisatIlta: 0, lisatYo: 0,
  ylityoVrk50: 0, ylityoVrk100: 0, ylityoVko50: 0, ylityoVko100: 0,
  atv: 0, matk: 0, paivaraha: 'ei', ateriakorvaus: 0,
  km: 0, tyokalukorvaus: 0, km_selite: '', huom: '', memo: '',
});
...
export const calculateTotals = (rows) => {
  ...
  if (!Array.isArray(rows) || rows.length === 0) return zero;
  return rows.reduce((acc, r) => {
    Object.keys(zero).forEach(k => { acc[k] += Number(r[k] || 0); });
    return acc;
  }, { ...zero });
};