  ...
  <div style="display: flex; height: 100vh; font-family: sans-serif;">
    ...
    <div style="width: 320px; border-right: 1px solid #ddd; padding: 16px; overflow-y: auto; flex-shrink: 0;">
      ...
      <div style="display: flex; gap: 4px; margin-bottom: 16px;">
        <button v-for="tab in TABS" :key="tab.key" @click="switchTab(tab.key)" :style="{
      ...
      <form @submit.prevent="handleUpload" style="margin-bottom: 24px;">
        <div style="margin-bottom: 8px;">
          <input v-model="documentName" :placeholder="t('documentName')" required
    ...
    <div
      style="flex: 1; background: #f9fafb; overflow-y: auto; display: flex; flex-direction: column; align-items: center;">

      <template v-if="activeTab === 'pdf' && pdfUrl">
        <div
          style="padding: 12px 0; display: flex; align-items: center; gap: 12px; position: sticky; top: 0; background: #f9fafb; z-index: 10; width: 100%; justify-content: center; border-bottom: 1px solid #e5e7eb;">
          <button @click="pageNumber = Math.max(1, pageNumber - 1)" :disabled="pageNumber <= 1"
          ...
          <button @click="pageNumber = Math.min(numPages, pageNumber + 1)" :disabled="pageNumber >= numPages"
            style="padding: 4px 12px; cursor: pointer; color: #33353a;">
            {{ t('nextPage') }}
          </button>
        </div>
      ...
      <div v-if="activeTab === 'word' && wordHtml" v-html="wordHtml"
      ...
      <template v-if="activeTab === 'excel' && activeSheet && sheetData[activeSheet]">
        <div style="width: 100%;">
          <div v-if="sheetNames.length > 1"
            style="display: flex; gap: 4px; padding: 8px 16px; border-bottom: 1px solid #e5e7eb; background: #f9fafb; flex-wrap: wrap;">
            <button v-for="name in sheetNames" :key="name" @click="activeSheet = name" :style="{
          ...
          <div style="overflow-x: auto; overflow-y: auto; padding: 16px;">
            <table style="border-collapse: collapse; font-size: 13px; white-space: nowrap; background: #fff;">
              <tr v-for="(row, rIdx) in sheetData[activeSheet]" :key="rIdx">
                <td v-for="(cell, cIdx) in row" :key="cIdx"
                  style="border: 1px solid #e5e7eb; padding: 4px 10px; min-width: 80px; min-height: 24px; color: #111; background: #fff;">
                  {{ cell?.value ?? '' }}
                </td>
              </tr>
            </table>
